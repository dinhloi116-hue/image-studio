(function(){
  function initPreviewModule(){
    ...
    function overlayHandlePoints(){
      const r=overlayRect(),cx=(r.left+r.right)/2,cy=(r.top+r.bottom)/2;
      return{
        nw:{x:r.left,y:r.top},n:{x:cx,y:r.top},ne:{x:r.right,y:r.top},
        e:{x:r.right,y:cy},se:{x:r.right,y:r.bottom},s:{x:cx,y:r.bottom},
        sw:{x:r.left,y:r.bottom},w:{x:r.left,y:cy}
      };
    }
    function drawOverlayHandles(){
      if(!state.overlayImage||!state.overlay.visible)return;
      const r=overlayRect(),hs=Math.max(14,Math.min(24,canvas.width/62)),pts=overlayHandlePoints();
      ctx.save();ctx.setLineDash([9,6]);ctx.strokeStyle='rgba(34,197,94,.98)';ctx.lineWidth=2;
    ...
    function hitOverlayHandle(p){
      if(!state.overlayImage||!state.overlay.visible)return '';
      const pts=overlayHandlePoints(),hs=Math.max(18,Math.min(32,canvas.width/48));
      for(const key of ['nw','n','ne','e','se','s','sw','w']){
        const h=pts[key];if(Math.abs(p.x-h.x)<=hs&&Math.abs(p.y-h.y)<=hs)return key;
      }
      return '';
    }
    ...
    function resizeOverlayFromHandle(handle,p){
      const d=state.drag,o=state.overlay,minSize=Math.max(24,canvas.width*.018);
      const x0=d.startOverlayX,y0=d.startOverlayY,w0=Math.max(minSize,d.startOverlayW),h0=Math.max(minSize,d.startOverlayH),x1=x0+w0,y1=y0+h0;
      let nx=x0,ny=y0,nw=w0,nh=h0;
      if(handle==='e')nw=Math.max(minSize,p.x-x0);
      else if(handle==='w'){nx=Math.min(p.x,x1-minSize);nw=x1-nx;}
      else if(handle==='s')nh=Math.max(minSize,p.y-y0);
      else if(handle==='n'){ny=Math.min(p.y,y1-minSize);nh=y1-ny;}
      else{
        const aspect=w0/Math.max(1,h0),minScale=Math.max(minSize/w0,minSize/h0);
        let anchorX=x0,anchorY=y0,cw=w0,ch=h0;
        if(handle==='se'){anchorX=x0;anchorY=y0;cw=p.x-anchorX;ch=p.y-anchorY;}
        if(handle==='nw'){anchorX=x1;anchorY=y1;cw=anchorX-p.x;ch=anchorY-p.y;}
        if(handle==='ne'){anchorX=x0;anchorY=y1;cw=p.x-anchorX;ch=anchorY-p.y;}
        if(handle==='sw'){anchorX=x1;anchorY=y0;cw=anchorX-p.x;ch=p.y-anchorY;}
        let scale=Math.max(minScale,Math.max(cw/w0,ch/h0));
        if(!Number.isFinite(scale))scale=1;
        nw=Math.max(minSize,w0*scale);nh=Math.max(minSize,nw/aspect);
        if(handle==='nw'){nx=x1-nw;ny=y1-nh;}
        else if(handle==='ne'){nx=x0;ny=y1-nh;}
        else if(handle==='sw'){nx=x1-nw;ny=y0;}
        else{nx=x0;ny=y0;}
      }
      const maxW=canvas.width*2,maxH=canvas.height*2;
      if(nw>maxW){const k=maxW/nw;nw=maxW;if(handle.length===2)nh*=k;}
      if(nh>maxH){const k=maxH/nh;nh=maxH;if(handle.length===2)nw*=k;}
    ...
    canvas.addEventListener('mousedown',e=>{
      ...
      if(overlayHandle){
        const o=state.overlay;state.drag.active=true;state.drag.mode='overlay-resize-'+overlayHandle;state.drag.startX=p.x;state.drag.startY=p.y;state.drag.startOverlayX=o.x;state.drag.startOverlayY=o.y;state.drag.startOverlayW=o.w;state.drag.startOverlayH=o.h;
        canvas.style.cursor=overlayCursor(overlayHandle);canvas.classList.add('dragging');setStatus('Đang co giãn ảnh dán thêm...');
    ...
    window.addEventListener('mouseup',()=>{if(state.drag.active)setStatus(state.drag.mode.startsWith('overlay-')?'Đã cập nhật vị trí/kích thước ảnh dán thêm.':'Đã cập nhật vị trí/kích thước tên số.');state.drag.active=false;canvas.classList.remove('dragging');canvas.style.cursor='default'});
