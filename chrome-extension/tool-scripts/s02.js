(function(){
  function initPreviewModule(){
    ...
    const state={
      ...
      drag:{active:false,mode:'move',offsetX:0,offsetY:0,startY:0,startW:0,startH:0,startNameCm:4,startNumCm:22,startGapCm:2}
    };
    ...
    function renderPreview(guides=true){
      ...
      const L=getLayout();
      drawSpacedText(L.name,L.centerX,L.nameBaseline,state.nameFontFamily,L.nameFontPx,L.nameTrackingPx,els.nameColor.value,L.strokePx,els.strokeColor.value,'center','700');
      drawSpacedText(L.number,L.centerX,L.numberBaseline,state.numberFontFamily,L.numberFontPx,L.numberTrackingPx,els.numberColor.value,L.strokePx,els.strokeColor.value,'center','900');
      if(guides){drawRuler();drawHandles(L);}
    }
    ...
    window.addEventListener('mouseup',()=>{if(state.drag.active)setStatus('Đã cập nhật vị trí/kích thước tên số.');state.drag.active=false;canvas.classList.remove('dragging')});
