  ...
  async function encode(x,idx,total,forceFmt=''){
    ...
    try{
      ...
      let blob=await new Promise(async(resolve,reject)=>{
        ...
        progressTimer=setInterval(()=>{x.progress=x.d?Math.min(99,v.currentTime/x.d*100):0;$('vc5Bar').style.width=((idx+x.progress/100)/Math.max(1,total)*100)+'%';setStatus(`Đang nén ${idx+1}/${total}: ${x.name} • ${Math.round(x.progress)}%`)},250);
