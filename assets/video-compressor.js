(()=>{
  if(window.__VIDCOMP_R197)return; window.__VIDCOMP_R197=1;
  ...
  @media(max-width:980px){.vc5-layout{grid-template-columns:1fr}.vc5-previews{grid-template-columns:1fr}.vc5-ratios{grid-template-columns:repeat(3,1fr)}}
  ...
  sec.innerHTML=`<div class="vc5-layout">
    <aside class="vc5-panel panel">
      <div class="vc5-card">
        <h2>🎬 Nén video</h2>
        <div class="vc5-note">Không cắt video. Tool luôn nén lại từ <b>file gốc đầy đủ</b> và kiểm tra thời lượng đầu ra trước khi cho tải.</div>
      ...
      <div class="vc5-card">
        ...
        <div id="vc5Summary" class="vc5-summary"><span class="vc5-big">80%</span> dung lượng gốc • giảm khoảng 20%</div>
      </div>
      <div class="vc5-card">
        <h3>2. Nén</h3>
      ...
      <details class="vc5-card vc5-advanced"><summary>Tùy chọn nâng cao</summary>
        ...
        <div class="vc5-note" style="margin-top:10px">Mẹo: muốn ít mờ nhất hãy để <b>Giữ nguyên độ phân giải</b>, dùng 80–90%. Chỉ hạ xuống 720p/480p khi cần file rất nhỏ.</div>
      </details>
    </aside>
  ...
  async function encode(x,idx,total,forceFmt='',forcedVideo=0,attempt=1){
    ...
    try{
      let videoTrack=null;
      const sameSize=ow===x.w-(x.w%2)&&oh===x.h-(x.h%2), canDirect=typeof v.captureStream==='function'&&sameSize;
      if(canDirect){rawStream=v.captureStream();videoTrack=rawStream.getVideoTracks()[0]||null}
  ...
  function setPct(v){v=Math.max(20,Math.min(95,+v||80));$('vc5Pct').value=v;$('vc5PctNum').value=v;$$('.vc5-ratio').forEach(b=>b.classList.toggle('active',+b.dataset.r===v));render()}
  ...
  window.videoCompressor={S,addFiles,runAll,runOne,show,version:'R19.7-hard-cap'};render();
})();