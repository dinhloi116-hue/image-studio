(()=>{
  if(window.__VIDCOMP_R199)return; window.__VIDCOMP_R199=1;
    ...
    <aside class="vc5-panel panel">
      ...
      <details class="vc5-card vc5-advanced"><summary>Tùy chọn nâng cao</summary>
        <div class="vc5-fields">
          ...
          <label>Định dạng<select id="vc5Fmt"><option value="mp4" selected>MP4 chuẩn Shopee (H.264 + AAC)</option></select></label>
        </div>
        <div class="vc5-note" style="margin-top:10px">Âm thanh mặc định được giữ ở <b>128 kbps</b>. Tool nén phần hình trước, sau đó <b>ghép lại âm thanh trực tiếp từ file gốc bằng FFmpeg</b>. File cuối phải có âm thanh thật mới được phép tải.</div>
      </details>
    </aside>
  ...
  function outputMime(force=''){let mp=['video/mp4;codecs=avc1.42E01E','video/mp4'];let wb=['video/webm;codecs=vp9','video/webm;codecs=vp8','video/webm'];let all=force==='webm'?wb:[...mp,...wb];return all.find(m=>MediaRecorder.isTypeSupported(m))||''}
  ...
  const FFW={worker:null,loaded:false,loadPromise:null,pending:new Map(),seq:1,urls:[]};
  function assetBase(){return location.protocol==='file:'?'https://dinhloi116-hue.github.io/image-studio/assets/':new URL('/image-studio/assets/',location.origin).href}
  async function blobURL(url,type){let r=await fetch(url,{cache:'force-cache'});if(!r.ok)throw Error(`Không tải được ${url} (${r.status})`);let b=await r.blob();let u=URL.createObjectURL(new Blob([b],{type}));FFW.urls.push(u);return u}
  function ffCall(type,data,transfer=[]){return new Promise((resolve,reject)=>{let id=FFW.seq++;FFW.pending.set(id,{resolve,reject});FFW.worker.postMessage({id,type,data},transfer)})}
  async function ensureFFmpeg(){
    if(FFW.loaded)return; if(FFW.loadPromise)return FFW.loadPromise;
    FFW.loadPromise=(async()=>{
      setStatus('Đang tải bộ ghép âm thanh FFmpeg ~31 MB (chỉ lần đầu)...');
      const base=assetBase();
      const [workerURL,coreURL,wasmURL]=await Promise.all([
        blobURL(base+'ffmpeg-worker-r199.js?v=r199','text/javascript'),
        blobURL(base+'ffmpeg-core.js?v=0.12.10','text/javascript'),
        blobURL(base+'ffmpeg-core.wasm?v=0.12.10','application/wasm')
      ]);
      FFW.worker=new Worker(workerURL);
      FFW.worker.onmessage=({data:{id,type,data}})=>{
        if(type==='log'){console.debug('ffmpeg',data);return}
        if(type==='progress'){return}
        const q=FFW.pending.get(id);if(!q)return;FFW.pending.delete(id);type==='error'?q.reject(Error(data)):q.resolve(data)
      };
      FFW.worker.onerror=e=>{for(const [,q] of FFW.pending)q.reject(Error('FFmpeg worker lỗi: '+(e.message||'không xác định')));FFW.pending.clear()};
      await ffCall('load',{coreURL,wasmURL,workerURL:''});FFW.loaded=true;
    })().catch(e=>{FFW.loadPromise=null;throw Error('Không thể tải bộ ghép âm thanh FFmpeg. Hãy kiểm tra mạng rồi thử lại. '+e.message)});
    return FFW.loadPromise;
  }
  async function ffWrite(path,blob){let u=new Uint8Array(await blob.arrayBuffer());await ffCall('write',{path,bytes:u},[u.buffer])}
  async function ffDelete(path){try{await ffCall('delete',{path})}catch(_){}}
  async function verifyAudio(blob){
    let AC=window.AudioContext||window.webkitAudioContext,ac=null;
    if(!AC)throw Error('Trình duyệt không hỗ trợ kiểm tra âm thanh.');
    try{ac=new AC();let ab=await blob.arrayBuffer();let buf=await ac.decodeAudioData(ab.slice(0));if(!buf||!buf.numberOfChannels||buf.duration<.2)return false;let peak=0;for(let c=0;c<buf.numberOfChannels;c++){let d=buf.getChannelData(c),step=Math.max(1,Math.floor(d.length/12000));for(let i=0;i<d.length;i+=step){let v=Math.abs(d[i]);if(v>peak)peak=v;if(peak>.00005)break}}return peak>.00001}catch(e){console.warn('Audio verify failed',e);return false}finally{try{await ac?.close()}catch(_){}}
  }
  async function muxOriginalAudio(x,videoBlob,videoMime,usedVideo,idx,total){
    const c=cfg(); if(c.a<=0)return {blob:videoBlob,mime:videoMime,audioOk:false};
    await ensureFFmpeg();
    const tag=`${Date.now()}_${Math.random().toString(36).slice(2,7)}`,vExt=videoMime.includes('mp4')?'mp4':'webm',srcExt=(x.name.match(/\.([a-z0-9]+)$/i)||[])[1]||'mp4';
    const vin=`v_${tag}.${vExt}`,src=`s_${tag}.${srcExt}`,out=`o_${tag}.mp4`;
    try{
      setStatus(`Đang ghép âm thanh gốc ${idx+1}/${total}: ${x.name}`);
      await ffWrite(vin,videoBlob);await ffWrite(src,x.file);
      let args=['-i',vin,'-i',src,'-map','0:v:0','-map','1:a:0?'];
      if(videoMime.includes('mp4'))args.push('-c:v','copy');
      else args.push('-c:v','libx264','-preset','ultrafast','-b:v',`${Math.max(180,Math.round(usedVideo/1000))}k`,'-pix_fmt','yuv420p');
      args.push('-c:a','aac','-b:a',`${c.a}k`,'-movflags','+faststart',out);
      const code=await ffCall('exec',{args,timeout:Math.max(60000,(x.d*4+30)*1000)});if(code!==0)throw Error('FFmpeg không ghép được âm thanh (mã '+code+').');
      const data=await ffCall('read',{path:out});const blob=new Blob([data.buffer],{type:'video/mp4'});
      const audioOk=await verifyAudio(blob);if(!audioOk)throw Error('File sau nén không có âm thanh thật. Tool đã chặn tải để tránh xuất video bị mất tiếng.');
      return {blob,mime:'video/mp4',audioOk:true};
    }finally{await ffDelete(vin);await ffDelete(src);await ffDelete(out)}
  }
  async function calibrateBitrate(x,idx,total,mt){
    if(x.d<5)return bitrate(x,mt).video;
    const c=cfg(),base=bitrate(x,mt).video,desired=targetBytes(x)*(c.mode==='mb'?.86:1)*8/Math.max(.1,x.d),sampleSec=Math.min(3.5,Math.max(2,x.d*.12));
    const start=Math.max(0,Math.min(x.d*.18,Math.max(0,x.d-sampleSec-.5)));
    const {v,url}=freshVideo(x.file);await waitReady(v);const [ow,oh]=dims(x);let stream=null,rawStream=null,drawTimer=null;
    try{
      ...
      let videoTrack=null,sameSize=ow===x.w-(x.w%2)&&oh===x.h-(x.h%2);if(typeof v.captureStream==='function'){try{rawStream=v.captureStream()}catch(_){}}
      ...
      const ms=new MediaStream([videoTrack]),rec=new MediaRecorder(ms,{mimeType:mt,videoBitsPerSecond:base}),chunks=[];let timer=null,aborted=false;
      setStatus(`Đang đo bitrate hình ${idx+1}/${total}: ${x.name} • ~${sampleSec.toFixed(1)} giây`);
      const blob=await new Promise(async(resolve,reject)=>{rec.ondataavailable=e=>{if(e.data?.size)chunks.push(e.data)};rec.onerror=()=>reject(Error('Không đo được bitrate video'));rec.onstop=()=>{clearInterval(timer);if(aborted)return reject(Object.assign(Error('Đã dừng'),{name:'AbortError'}));let b=new Blob(chunks,{type:mt});b.size?resolve(b):reject(Error('Không đo được bitrate video'))};timer=setInterval(()=>{if(v.currentTime>=start+sampleSec||v.ended){try{if(rec.state!=='inactive')rec.stop()}catch(_){}}},80);S.stopCurrent=()=>{aborted=true;try{v.pause()}catch(_){}try{if(rec.state!=='inactive')rec.stop()}catch(_){}};try{rec.start(500);await v.play()}catch(e){clearInterval(timer);reject(Error('Không chạy được bước đo bitrate'))}});
      const mediaSec=Math.max(.5,Math.min(sampleSec,Math.max(0,v.currentTime-start))),actual=blob.size*8/mediaSec;let factor=desired/Math.max(1,actual);factor=Math.max(.25,Math.min(3.2,factor));return Math.max(180000,Math.min(100000000,base*factor));
    }finally{clearInterval(drawTimer);S.stopCurrent=null;try{v.pause()}catch(_){}try{rawStream?.getTracks().forEach(t=>t.stop())}catch(_){}try{stream?.getTracks().forEach(t=>t.stop())}catch(_){}URL.revokeObjectURL(url)}
  }
  async function encode(x,idx,total,forceFmt='',forcedVideo=0,attempt=1){
    const c=cfg(),mt=outputMime(forceFmt);if(!mt)throw Error('Chrome không hỗ trợ mã hóa video trên máy này');
    const tunedVideo=forcedVideo>0?forcedVideo:await calibrateBitrate(x,idx,total,mt);
    const {v,url}=freshVideo(x.file);await waitReady(v);const [ow,oh]=dims(x);let stream=null,canvas=null,drawTimer=null,rawStream=null;
    try{
      let videoTrack=null,sameSize=ow===x.w-(x.w%2)&&oh===x.h-(x.h%2);if(typeof v.captureStream==='function'){try{rawStream=v.captureStream()}catch(_){}}
      if(rawStream&&sameSize)videoTrack=rawStream.getVideoTracks()[0]||null;
      ...
      const ms=new MediaStream([videoTrack]),rec=new MediaRecorder(ms,{mimeType:mt,videoBitsPerSecond:tunedVideo}),chunks=[];let aborted=false,settled=false,progressTimer=null,hardTimer=null;
      const blob=await new Promise(async(resolve,reject)=>{const cleanup=()=>{clearInterval(progressTimer);clearTimeout(hardTimer);S.stopCurrent=null};const fail=e=>{if(settled)return;settled=true;cleanup();reject(e)};rec.ondataavailable=e=>{if(e.data&&e.data.size)chunks.push(e.data)};rec.onerror=()=>fail(Error('Lỗi mã hóa video'));rec.onstop=()=>{if(settled)return;settled=true;cleanup();if(aborted)return reject(Object.assign(Error('Đã dừng'),{name:'AbortError'}));let b=new Blob(chunks,{type:mt});b.size?resolve(b):reject(Error('Không tạo được file hình đầu ra'))};v.onended=()=>{try{if(rec.state!=='inactive')rec.stop()}catch(e){fail(e)}};progressTimer=setInterval(()=>{x.progress=x.d?Math.min(94,v.currentTime/x.d*94):0;$('vc5Bar').style.width=((idx+x.progress/100)/Math.max(1,total)*100)+'%';setStatus(`${attempt>1?'Đang căn lại giới hạn • ':''}Đang nén hình ${idx+1}/${total}: ${x.name} • ${Math.round(x.progress)}%`)},250);hardTimer=setTimeout(()=>fail(Error('Nén quá thời gian dự kiến, đã dừng để tránh treo')),Math.max(30000,(x.d*1.8+20)*1000));S.stopCurrent=()=>{aborted=true;try{v.pause()}catch(_){}try{if(rec.state!=='inactive')rec.stop()}catch(_){} };try{rec.start(1000);await v.play()}catch(e){aborted=true;try{if(rec.state!=='inactive')rec.stop()}catch(_){}fail(Error('Chrome không phát được video nguồn/codec này'))}});
      const pm=await probeBlob(blob),tol=Math.max(.75,x.d*.03),delta=Math.abs(pm.d-x.d);if(delta>tol){URL.revokeObjectURL(pm.url);throw Error(`Sai thời lượng phần hình: gốc ${x.d.toFixed(2)}s nhưng file nén ${pm.d.toFixed(2)}s.`)}
      return {blob,mime:mt,dur:pm.d,url:pm.url,w:pm.w,h:pm.h,usedVideo:tunedVideo};
    }finally{clearInterval(drawTimer);try{v.pause()}catch(_){}try{rawStream?.getTracks().forEach(t=>t.stop())}catch(_){}try{stream?.getTracks().forEach(t=>t.stop())}catch(_){}URL.revokeObjectURL(url)}
  }
  async function finalizeOutput(x,videoResult,idx,total){
    let merged=await muxOriginalAudio(x,videoResult.blob,videoResult.mime,videoResult.usedVideo,idx,total);if(videoResult.url)URL.revokeObjectURL(videoResult.url);
    const pm=await probeBlob(merged.blob),tol=Math.max(.75,x.d*.03),delta=Math.abs(pm.d-x.d);if(delta>tol){URL.revokeObjectURL(pm.url);throw Error(`Sai thời lượng: gốc ${x.d.toFixed(2)}s nhưng file cuối ${pm.d.toFixed(2)}s. Đã chặn tải.`)}
    const c=cfg(),target=targetBytes(x),source=x.file.size,overHard=c.mode==='mb'&&merged.blob.size>target,tooBig=merged.blob.size>=source*.98,farOver=c.mode!=='mb'&&merged.blob.size>target*1.30;
    if(overHard||tooBig||farOver){URL.revokeObjectURL(pm.url);let e=Error(overHard?`File cuối ${fmt(merged.blob.size)} vượt trần cứng ${fmt(target)}.`:tooBig?`File cuối ${fmt(merged.blob.size)} không nhỏ hơn file gốc ${fmt(source)}.`:`File cuối ${fmt(merged.blob.size)} vượt quá xa mục tiêu ${fmt(target)}.`);e.code=overHard?'HARDCAP':tooBig?'OVERSIZE':'MISSTARGET';e.actualSize=merged.blob.size;e.targetSize=target;e.usedVideo=videoResult.usedVideo;throw e}
    return {blob:merged.blob,mime:merged.mime,dur:pm.d,url:pm.url,w:pm.w,h:pm.h,audioOk:merged.audioOk,usedVideo:videoResult.usedVideo};
  }
  async function runItem(i,idx=0,total=1){let x=S.items[i];if(!x)return;if(x.outUrl){URL.revokeObjectURL(x.outUrl);x.outUrl=''}x.out=null;x.valid=false;x.err='';x.codecNote='';x.audioOk=false;x.work=true;x.progress=0;render();try{let r,forcedVideo=0,attempt=1;for(;;){let vr=null;try{vr=await encode(x,idx,total,'',forcedVideo,attempt);x.progress=95;render();r=await finalizeOutput(x,vr,idx,total);break}catch(e){if(vr?.url)try{URL.revokeObjectURL(vr.url)}catch(_){}if(e.name==='AbortError')throw e;const c=cfg();if(c.mode==='mb'&&e.code==='HARDCAP'&&attempt<4){const ratio=e.targetSize/Math.max(1,e.actualSize);forcedVideo=Math.max(180000,(e.usedVideo||forcedVideo||bitrate(x).video)*ratio*.88);x.codecNote=`Vượt trần lần ${attempt} → tự nén lại từ file gốc`;setStatus(`${x.name}: ${fmt(e.actualSize)} > trần ${fmt(e.targetSize)}. Đang nén lại từ file gốc...`);attempt++;continue}throw e}}x.out=r.blob;x.outUrl=r.url;x.outDur=r.dur;x.mime='video/mp4';x.audioOk=!!r.audioOk;x.valid=true;x.progress=100;if(cfg().a>0&&!x.audioOk){x.valid=false;throw Error('Không xác nhận được âm thanh trong file cuối. Đã chặn tải.')}if(cfg().mode==='mb'&&x.out.size>targetBytes(x)){x.valid=false;throw Error(`Lỗi an toàn: file ${fmt(x.out.size)} vẫn vượt trần ${fmt(targetBytes(x))}. Đã chặn tải.`)}}catch(e){if(e.name==='AbortError')throw e;x.err=e.message}finally{x.work=false;render()}}
  ...
  function outName(x){return x.name.replace(/\.[^.]+$/,'')+`_compressed.mp4`}
  ...
  window.videoCompressor={S,addFiles,runAll,runOne,show,version:'R19.9-ffmpeg-audio'};syncModeButtons();render();
})();
