...
rep('if(window.__VIDCOMP_R196)return; window.__VIDCOMP_R196=1;','if(window.__VIDCOMP_R197)return; window.__VIDCOMP_R197=1;','guard')
rep("else {sm.innerHTML=`Tối đa <span class=\"vc5-big\">${c.mb} MB</span> mỗi video`;sm.className='vc5-summary'}","else {sm.innerHTML=`Giới hạn cứng <span class=\"vc5-big\">≤ ${c.mb} MB</span> mỗi video • chỉ cho tải khi không vượt`;sm.className='vc5-summary'}",'summary')
rep('<span class="vc5-badge">Mục tiêu ${fmt(t)}</span>','<span class="vc5-badge">${cfg().mode===\'mb\'?\'Trần cứng ≤ \'+fmt(t):\'Mục tiêu \'+fmt(t)}</span>','badge')
rep("const c=cfg(),base=bitrate(x,mt).video,desired=targetBytes(x)*8/Math.max(.1,x.d),sampleSec=","const c=cfg(),base=bitrate(x,mt).video,desired=targetBytes(x)*(c.mode==='mb'?.88:1)*8/Math.max(.1,x.d),sampleSec=",'calibration')
rep("async function encode(x,idx,total,forceFmt=''){","async function encode(x,idx,total,forceFmt='',forcedVideo=0,attempt=1){",'encode signature')
rep('const tunedVideo=await calibrateBitrate(x,idx,total,mt);','const tunedVideo=forcedVideo>0?forcedVideo:await calibrateBitrate(x,idx,total,mt);','forced bitrate')
rep("setStatus(`Đang nén ${idx+1}/${total}: ${x.name} • ${Math.round(x.progress)}%`)","setStatus(`${attempt>1?'Đang căn lại giới hạn • ':''}Đang nén ${idx+1}/${total}: ${x.name} • ${Math.round(x.progress)}%`)",'progress')
old="const target=targetBytes(x),source=x.file.size,tooBig=blob.size>=source*.98,farOver=target>0&&blob.size>target*1.30;if(tooBig||farOver){URL.revokeObjectURL(pm.url);const e=Error(tooBig?`Kết quả ${fmt(blob.size)} không nhỏ hơn file gốc ${fmt(source)}. Tool đã chặn, không coi đây là nén thành công.`:`Kết quả ${fmt(blob.size)} vượt quá xa mục tiêu ${fmt(target)}.`);e.code=tooBig?'OVERSIZE':'MISSTARGET';e.usedMime=mt;throw e}"
new="const target=targetBytes(x),source=x.file.size,hardCap=c.mode==='mb',overHard=hardCap&&blob.size>target,tooBig=blob.size>=source*.98,farOver=!hardCap&&target>0&&blob.size>target*1.30;if(overHard||tooBig||farOver){URL.revokeObjectURL(pm.url);let msg=overHard?`Kết quả ${fmt(blob.size)} vượt trần cứng ${fmt(target)}. Tool sẽ không cho tải file này.`:tooBig?`Kết quả ${fmt(blob.size)} không nhỏ hơn file gốc ${fmt(source)}. Tool đã chặn, không coi đây là nén thành công.`:`Kết quả ${fmt(blob.size)} vượt quá xa mục tiêu ${fmt(target)}.`;const e=Error(msg);e.code=overHard?'HARDCAP':tooBig?'OVERSIZE':'MISSTARGET';e.usedMime=mt;e.usedVideo=tunedVideo;e.actualSize=blob.size;e.targetSize=target;throw e}"
rep(old,new,'output validation')
...
rep(old2,new2,'runItem')
rep("version:'R19.6-calibrated'","version:'R19.7-hard-cap'",'version')
p.write_text(s,encoding='utf-8')
for hp in [Path('index.html'),Path('direct/index.html')]:
    if hp.exists():
        h=hp.read_text(encoding='utf-8')
        h=h.replace('video-compressor.js?v=r196-calibrated-safe','video-compressor.js?v=r197-hard-cap')
